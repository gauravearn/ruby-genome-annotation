def rubyAnnotationparse
  ...
  start_split = `for j in $(cat sample_file.txt | cut -f 4 | grep "^[0-9]"); do echo $j; done | head -n 10`.split.map{|each| each.to_i}
  end_split = `for j in $(cat sample_file.txt | cut -f 5 | grep "^[0-9]"); do echo $j; done | head -n 10`.split.map{|each| each.to_i}
